...
The running example is **Arxie**, an academic research assistant. The abbreviation **RA** is retained where a shorter label improves readability. [Lewis et al., 2020]
...
2. **It quantifies the value of model improvement.** If Maverick's citation precision could be improved from 0.82 to 0.87 through fine-tuning (a topic addressed in Chapter 6), the self-hosting option becomes viable and saves $30,000+/month at scale.
...
It does not address model *customization* (fine-tuning, deferred to Chapter 6), *evaluation infrastructure at scale* (the subject of Chapter 3), or *system architecture design* (the subject of Chapter 4).
The model selection procedure produces a candidate and a production readiness plan; the subsequent chapters address how to validate that candidate continuously, how to build the system around it, how to operate it in production, and only then how to decide whether model customization is justified.
